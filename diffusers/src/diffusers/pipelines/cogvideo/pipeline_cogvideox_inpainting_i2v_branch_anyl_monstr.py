...
class CogVideoXI2VDualInpaintAnyLPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def prepare_video_latents(
        self, video, batch_size, height, width, dtype, device, generator, do_classifier_free_guidance
    ):
        ...
        if video.shape[1] == 4:
            ...
            video_latents = self._encode_vae_video(video, generator=generator)

        # duplicate mask and masked_image_latents for each generation per prompt, using mps friendly method
    
        if video_latents.shape[0] < batch_size:
            if not batch_size % video_latents.shape[0] == 0:
                raise ValueError(
                    "The passed images and the required batch size don't match. Images are supposed to be duplicated"
                    f" to a total batch size of {batch_size}, but {video_latents.shape[0]} images were passed."
                    " Make sure the number of images that you pass is divisible by the total requested batch size."
                )
            video_latents = video_latents.repeat(batch_size // video_latents.shape[0], 1, 1, 1, 1)
    ...
    def __call__(
        ...
        condition_video: List[PIL.Image.Image] = None,
        condition_video_latents: torch.Tensor = None,
        strength: float = 1.0,
        ...
        for _ in range(1):
            ...
            if isinstance(condition_video, (list, tuple)):
                window_condition_video = condition_video[start_frame:end_frame]
            else:
                window_condition_video = condition_video[:, start_frame:end_frame]
            ...
            if window_idx == 0:
                ...
                if -int((num_frames - stride) // self.vae_scale_factor_temporal) < 0:
                    image_ = self.latents[:, -int((num_frames - stride) // self.vae_scale_factor_temporal) - 1:-int((num_frames - stride) // self.vae_scale_factor_temporal) + 1 - 1]
            ...
            # 7. Prepare condition video latents
            condition_video = self.video_processor.preprocess_video(window_condition_video, height=height, width=width)
            if condition_video_latents is not None:
                condition_video_latents = None
                       
            condition_video_latents = self.prepare_video_latents(
                condition_video,
                batch_size * num_videos_per_prompt,
                height,
                width,
            ...
            with self.progress_bar(total=num_inference_steps) as progress_bar:
                for i, t in enumerate(timesteps):
                    ...
                    # branch block
                    latent_branch_input = condition_video_latents
                    ...
                    noise_pred, hidden_states_list = self.transformer(
                        ...
                        return_resample_mask=False, # previous was true!
                        return_dict=False,
                    )
                    ...
                    if window_idx < n_windows - 1:
                        if t.item() == timesteps[-1]:
                            ...
                            self.prev_window_states_dict = prev_window_states_dict
