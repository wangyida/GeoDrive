...
def render_2D_images_from_scene(outdir, scene, min_conf_thr=3, mask_sky=False,
    ...
    # import pdb; pdb.set_trace()
    # device = torch.device('cuda')
    # camera_traj,num_views = generate_traj_specified(c2ws[0:1], H, W, focals[0:1], principal_points[0:1], d_theta, d_phi, d_r,d_x*depth_avg/focals[0].item(),d_y*depth_avg/focals[0].item(),1, device)
    # camera_traj,num_views = generate_traj_from_c2ws(c2ws, H, W, focals, principal_points, len(c2ws), device)
    ...
    render_results, viewmask = run_render(pcd[0:1], rgbimg[0:1], ask[0:1], H, W, camera_traj,num_views, device=device, nbv=False)
