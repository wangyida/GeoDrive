...
def save_frames(data, output_dir, save_names=None, folder=None):
    ...
    for i in range(tensor_data.shape[0]):
        frame = tensor_data[i]
        frame_path = os.path.join(output_dir, f'{save_names[i]}.png')
...
def world_point_to_obj_nuscenes(poses, points, c2ws, k, r, elevation, device):
    """
    Transform camera poses and points to object-centric coordinates while maintaining RDF system.
    
    Args:
        poses: [N, 4, 4] Camera poses in RDF coordinate system(nuscenes coordinate system)
        points: [M, H, W, 3] 3D points or None, where M may not equal N(dust coordinate system)
        k: Index of reference camera
        r: Radius for centering
        elevation: Camera elevation angle in degrees
        device: torch device
        
    Returns:
        new_poses: [N, 4, 4] Transformed camera poses
        new_points: [M, H, W, 3] Transformed points (if points provided)
    """
    def calculate_mean_scale(poses2kth_pose, dust2kth):
        if len(poses2kth_pose) <= 1 or len(dust2kth) <= 1:
            return 1.
        nuscenes_trans = poses2kth_pose[1:, :3, 3]
        dust3r_trans = dust2kth[1:, :3, 3]
        scales = dust3r_trans.norm(dim=-1) / (nuscenes_trans.norm(dim=-1) + 1e-6)
        return scales.mean()

    # 1. First transform everything to kth camera coordinate system
    kth_pose = poses[k].to(torch.float64).to(device)  # Reference pose
    inv_kth_pose = torch.inverse(kth_pose).to(torch.float64)
    kth_pose_dust = c2ws[k].to(torch.float64).to(device)
    inv_kth_pose_dust = torch.inverse(kth_pose_dust).to(torch.float64)
    # kth_pose2dust = kth_pose_dust @ inv_kth_pose

    
    # Transform all poses to reference camera space
    poses2kth_pose = torch.bmm(
        inv_kth_pose.unsqueeze(0).expand_as(poses.to(torch.float64)), 
        poses.to(torch.float64)
    )
    # dust all poses to reference camera space
    dust2kth = torch.bmm(
        inv_kth_pose_dust.unsqueeze(0).expand_as(c2ws.to(torch.float64)), 
        c2ws.to(torch.float64)
    )
    # scale factor of nuscenes to dust3r--dust3r / nuscenes
    scale_factor = calculate_mean_scale(poses2kth_pose, dust2kth)
    poses2kth_pose_new = poses2kth_pose
    poses2kth_pose_new[:, :3, 3] *= scale_factor

    # 2. Create object-centric transformation
    # Convert elevation to radians and create rotation matrix
    elevation_rad = torch.deg2rad(torch.tensor(180-elevation)).to(device)
    sin_value = torch.sin(elevation_rad)
    cos_value = torch.cos(elevation_rad)
    
    # Rotation matrix for RDF coordinate system
    # Rotates around right (x) axis to achieve desired elevation
    R = torch.tensor([
        [1, 0, 0],
        [0, cos_value, -sin_value],  # Note: signs adjusted for RDF
        [0, sin_value, cos_value]
    ], device=device)
    
    # Translation to center object at distance r along forward axis
    t = torch.tensor([0, 0, r], device=device)
    
    # Combine into transformation matrix
    pose_obj = torch.eye(4, device=device)
    pose_obj[:3, :3] = R
    pose_obj[:3, 3] = t
    
    # 3. Apply object-centric transformation
    inv_obj_pose = torch.inverse(pose_obj).to(torch.float64)
    new_poses = torch.bmm(
        inv_obj_pose.unsqueeze(0).expand_as(poses2kth_pose_new), 
        poses2kth_pose_new
    )
    
    # 4. Transform points if provided
    if points is not None:
        M, H, W, _ = points.shape
        points_reshaped = points.view(M, H * W, 3)
        
        # Add homogeneous coordinate
        homogeneous_points = torch.cat([
            points_reshaped, 
            torch.ones(M, H*W, 1, device=device)
        ], dim=-1)
        
        # First transform to reference camera space
        points_cam = torch.bmm(
            inv_kth_pose_dust.unsqueeze(0).expand(M, -1, -1),
            homogeneous_points.transpose(1, 2).to(torch.float64)
        ).transpose(1, 2)
        
        # Then transform to object space
        new_points = torch.bmm(
            inv_obj_pose.unsqueeze(0).expand(M, -1, -1),
            points_cam.transpose(1, 2).to(torch.float64)  # Transpose to match batch matrix multiply dimensions
        ).transpose(1, 2)  # Transpose back to original shape
        
        # Remove homogeneous coordinate and reshape
        new_points = new_points[..., :3].view(M, H, W, 3)
        
        return new_poses.to(torch.float32), new_points.to(torch.float32)
    
    return new_poses.to(torch.float32)
