...
class VideoInpaintingDataset(Dataset):
    def __init__(
        ...
        condition_data_root: Optional[str] = None,
        video_data_root: Optional[str] = None,
        ...
        self.condition_data_root = Path(condition_data_root) if condition_data_root is not None else None
        self.video_data_root = Path(video_data_root) if video_data_root is not None else None
    ...
    def __getitem__(self, index):
        ...
        if self.load_mode == "online":
            ...
            try:
                ...
                # transform to BGR
                # video_frames = [cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) for frame in video_frames]
                # condition_video_frames = [cv2.cvtColor(frame, cv2.COLOR_RGB2BGR) for frame in condition_video_frames]
            ...
            # condition_video = condition_video[::int(fps//self.fps)]
    ...
    def _load_dataset_from_local_path(self):
        ...
        if not self.is_train:
            with open('/lpai/volumes/ad-vla-vol-ga/chenantong/codes/balanced_1k_selected_idx.txt', 'r') as f:
                self.selected_idx = [int(x) for x in f.read().split(',')]
        ...
        for sample in metadata:
            if 'waymo' in sample['gt_frames'][0]: # from waymo
                self.videos.append([gt_frame_path for gt_frame_path in sample["gt_frames"]])
                self.condition_videos.append([render_frame_path for render_frame_path in sample["render_frames"]])
                self.scene_idxs.append(sample["scene_idx"])
                print(f"waymo: {sample['render_frames'][0]}")
            else:
                self.videos.append([self.video_data_root / gt_frame_path for gt_frame_path in sample["gt_frames"]])
                self.condition_videos.append([self.condition_data_root / render_frame_path for render_frame_path in sample["render_frames"]])
                self.scene_idxs.append(sample["scene_idx"])

        if not self.is_train:
            self.prompts = [p for i, p in enumerate(self.prompts) if i in self.selected_idx]
            self.videos = [v for i, v in enumerate(self.videos) if i in self.selected_idx]
            self.condition_videos = [c for i, c in enumerate(self.condition_videos) if i in self.selected_idx]
            self.scene_idxs = [s for i, s in enumerate(self.scene_idxs) if i in self.selected_idx]
        ...
        if self.is_train:
            ...
            for example in examples:
                ...
                if frame > self.max_num_frames:
                    ...
                    assert video.shape[0] == condition_video.shape[0], "video and condition_video shape is not equal"

                assert video.shape[0] == self.max_num_frames, "video shape is not equal to max_num_frames"
                assert condition_video.shape[0] == self.max_num_frames, "condition_video shape is not equal to max_num_frames"
                ...
                condition_video = condition_video.permute(0, 2, 3, 1).numpy()

                for i in range(len(video)):
                    video[i] = cv2.cvtColor(video[i], cv2.COLOR_BGR2RGB)
                video = (video.astype(np.float32) / 127.5) - 1.0

                for i in range(len(condition_video)):
                    condition_video[i] = cv2.cvtColor(condition_video[i], cv2.COLOR_BGR2RGB)
                condition_video = (condition_video.astype(np.float32) / 127.5) - 1.0
             

                if random.random() < self.mix_train_ratio:
                    video, condition_video = video[:1], condition_video[:1]
                else:
                    if self.first_frame_gt:
                        video[0] = condition_video[0]
    
                pixel_values.append(torch.tensor(video).permute(0, 3, 1, 2))
                conditioning_pixel_values.append(torch.tensor(condition_video).permute(0, 3, 1, 2))
            ...
            for example in examples:
                ...
                condition_video=example["condition_video"] # frame, height, width, c
                frame, height, width, c = video.shape
                assert condition_video.shape[0] == frame, "condition_video shape is not equal to video shape"
                ...
                # resize video and condition_video to the nearest resolution
                video = torch.from_numpy(video).permute(0, 3, 1, 2)
                condition_video = torch.from_numpy(condition_video).permute(0, 3, 1, 2)
                nearest_res = self._find_nearest_resolution(video.shape[2], video.shape[3])
                if self.video_reshape_mode == 'center':
                ...
                assert len(condition_video) == len(video), f"condition_video shape is not equal to video shape for {example['prompt']}"

                if self.first_frame_gt:
                    condition_video[0] = video[0]

                video_ = [Image.fromarray(cv2.cvtColor(video[i], cv2.COLOR_BGR2RGB)) for i in range(len(video))]
                condition_video_ = [Image.fromarray(cv2.cvtColor(condition_video[i], cv2.COLOR_BGR2RGB)) for i in range(len(condition_video))]
                
                pixel_values.append(video_)
                conditioning_pixel_values.append(condition_video_)
...
def log_validation(
    ...
    for _ in range(args.num_validation_videos):
        ...
        condition_video = pipe.video_processor.preprocess_video(pipeline_args['condition_video'][0], height=video.shape[1], width=video.shape[2])
...
def main(args):
    ...
    accelerator = Accelerator(
        ...
        deepspeed_plugin=deepspeed_plugin # debug
    )
    ...
    validation_dataset = VideoInpaintingDataset(
        ...
        condition_data_root=args.val_condition_data_root,
        video_data_root=args.video_data_root,
        dataset_name=args.dataset_name,
        dataset_config_name=args.dataset_config_name,
        height=args.height,
    ...
    if not args.resume_from_checkpoint:
        ...
        if path is None:
            ...
            try:
                ...
                logger.info(f'failed loading from {os.path.join(args.output_dir, path)}')
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(models_to_accumulate):
                ...
                # conditioning_latents=torch.concat([conditioning_latents, masks], -3).to(dtype=weight_dtype) # we do not use masks
